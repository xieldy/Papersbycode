# -*- coding: utf-8 -*-
"""
Created on Thu Aug 27 09:44:56 2020
...
def DownloadPaper(paper_dic_i):
    paper_url = "https://proceedings.icml.cc/static/paper_files/icml/2020/"+str(paper_dic_i["sourceid"])+"-Paper.pdf"
    print(paper_dic_i["content"]["title"])
...
#   with open('C:\\Users\\maggie\\.spyder-py3\\CVPR2020\\'+paper_id,"wb") as code:
    papername = paper_dic_i["content"]["title"].replace(" ","_").replace(":","").replace("?","").replace("/","_").replace("|","_").replace(":","").replace('"','').replace("$","").replace("\\","_").replace("*","")
...
url="https://icml.cc/static/virtual/data/icml_papers.json"
...
#print(paper_dic)
for i in range(931,1085):#第736，930篇链接失效下不下来，range第一位是指从哪一篇开始
    print("第"+str(i)+"篇")
